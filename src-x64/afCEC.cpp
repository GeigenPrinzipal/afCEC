...
void afCECHartiganRemoveCluster(
    ...
    for (int j = 0; j < pointsNum; ++j) {
        ...
        if (clToRemove == cl) {
            ...
            for (std::set<int>::iterator it2 = activeClusters.begin(); it2 != activeClusters.end(); ++it2) {
                vec v = (points.submat(1, j, size(dim - 1, 1)) - m[*it2]->subvec(1, dim - 1)) / sqrt(card[*it2] + 1.0);
                for (int k = 0; k < dim; ++k) {
                    ...
                    if (k < dim - 1) v[k] = (points.at(k, j) - (*m[*it2])[k]) / sqrt(card[*it2] + 1.0);
                }
            }
            if (dEMinIncl < INFINITY) {
                vec v = (points.submat(1, j, size(dim - 1, 1)) - m[bestClIncl]->subvec(1, dim - 1)) / sqrt(card[bestClIncl] + 1.0);
                for (int k = 0; k < dim; ++k) {
                    vec vTmp = v;
                    ...
                    *A[bestClIncl][k] += (vA * vA.t());
                    ...
                    if (k < dim - 1) v[k] = (points.at(k, j) - (*m[bestClIncl])[k]) / sqrt(card[bestClIncl] + 1.0);
                }
    ...
    do {
        ...
        for (int i = 0; i < pointsNum; ++i) {
            ...
            if (activeClusters.find(cl) != activeClusters.end()) {
                ...
                double bestVarExcl;
                try {
                    vec v = (points.submat(1, i, size(dim - 1, 1)) - m[cl]->subvec(1, dim - 1)) / sqrt(card[cl] - 1.0);
                    for (int j = 0; j < dim; ++j) {
                        ...
                        if (j < dim - 1) v[j] = (points.at(j, i) - (*m[cl])[j]) / sqrt(card[cl] - 1.0);
                    }
                } catch (...) {
                    printf("WYWALAMY %d\n", cl);
                ...
                if (activeClusters.find(cl) != activeClusters.end()) {
                    ...
                    for (std::set<int>::iterator it = activeClusters.begin(); it != activeClusters.end(); ++it) {
                        if ((*it != cl) && (activeClusters.find(*it) != activeClusters.end())) {
                            vec v = (points.submat(1, i, size(dim - 1, 1)) - m[*it]->subvec(1, dim - 1)) / sqrt(card[*it] + 1.0);
                            for (int j = 0; j < dim; ++j) {
                                ...
                                if (j < dim - 1) v[j] = (points.at(j, i) - (*m[*it])[j]) / sqrt(card[*it] + 1.0);
                            }
                    ...
                    if (dEMinExcl + dEMinIncl < 0.0) {
                        ...
                        v = (points.submat(1, i, size(dim - 1, 1)) - m[bestClIncl]->subvec(1, dim - 1)) / sqrt(card[bestClIncl] + 1.0);
                        for (int j = 0; j < dim; ++j) {
                            vec vTmp = v;
                            ...
                            *A[bestClIncl][j] += (vA * vA.t());
                            ...
                            if (j < dim - 1) v[j] = (points.at(j, i) - (*m[bestClIncl])[j]) / sqrt(card[bestClIncl] + 1.0);
                        }
